...
- 스탬프는 미션 완료 시 증가하고 리워드 교환 시 감소하며, 모든 증감 내역은 StampTransaction으로 관리된다.
- 리워드는 보유 스탬프가 필요 수량(requiredStamps) 이상일 때만 교환 가능하며, 부족하면 교환할 수 없다.
- 리워드 교환이 완료되면 즉시 스탬프가 차감되고 교환 내역(RewardRedemption)이 기록된다.
